...
function makeWord(lettersObject) {
  const arr = [];
  const arrLetters = Object.keys(lettersObject);
  const arrNumbers = Object.values(lettersObject);
  arrLetters.forEach((letter, index) => {
    arrNumbers[index].forEach((position) => {
      arr[position] = letter;
    });
  });
...
class CssSelectorBuilder {
  ...
  element(value) {
    this.checkSingleUsage('element');
    this.selector += value;
    return this;
  }

  id(value) {
    this.checkSingleUsage('id');
    this.selector += `#${value}`;
    return this;
  }

  class(value) {
    const classBuilder = new CssSelectorBuilder();
    classBuilder.selector += `.${value}`;
    classBuilder.seen.class = true;
    return classBuilder;
  }

  attr(value) {
    this.checkSingleUsage('attr');
    this.selector += `[${value}]`;
    return this;
  }

  pseudoClass(value) {
    this.checkSingleUsage('pseudoClass');
    this.selector += `:${value}`;
    return this;
  }

  pseudoElement(value) {
    this.checkSingleUsage('pseudoElement');
    this.selector += `::${value}`;
    return this;
  }
  ...
  checkSingleUsage(selectorType) {
    if (this.seen[selectorType]) {
      throw new Error(`${selectorType} selector can only be specified once.`);
    }
    this.seen[selectorType] = true;
  }
}
